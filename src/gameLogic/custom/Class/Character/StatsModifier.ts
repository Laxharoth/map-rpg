...
export function applyModifiers(character:Character, modifiers:StatsModifier):void{
  const characterStats = character.calculatedStats as unknown as { [key:string]: Int};
  ...
  const characterResistance = character.calculatedResistance as unknown as { [key:string]: Int};
